...
const castFilterValue = (type, val) => {
    if (!casters[type])
        return val;
    const parsed = typeof val === "string" && val.match(/^\[.*\]$/)
        ? val.slice(1, -1).split(",")
        : val;
...
export function parseRequest(schema, params) {
    ...
    const go = (type, path = []) => {
        ...
        const relevantFilters = pickBy(filter ?? {}, (f, k) => (path.length === 0 && !k.includes(".")) ||
            (k.startsWith(`${path.join(".")}.`) &&
                !k.split(`${path.join(".")}.`)[1].includes(".")));
        const parsedFilters = {};
        Object.entries(relevantFilters).forEach(([key, val]) => {
            parsedFilters[path.length === 0 ? key : key.split(`${path.join(".")}.`)[1]] = val;
        });
        ...
        const included = parsedInclude
            .filter((i) => (path.length === 0 && !i.includes(".")) ||
        ...
        const order = sort && path.length === 0
            ? sort.split(",").map((field) => {
                ...
                if (!Object.keys(resDef.attributes).includes(parsedField)) {
                    throw new Error(`${parsedField} is not a valid attribute of ${type}`);
                }
