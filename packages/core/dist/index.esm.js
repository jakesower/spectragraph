import { uniqBy, get, omit, mapValues, pickBy, merge, orderBy, uniq, isEqual } from 'lodash-es';
import Ajv from 'ajv';
import addFormats from 'ajv-formats';
import { applyOrMap } from '@data-prism/utils';
import { defaultExpressionEngine as defaultExpressionEngine$1 } from '@data-prism/expressions';
	...
	if (!compiledValidator) {
		...
		const queryDefinition = (query) => ({
			...
			properties: mapValues(query.select, (def, prop) => {
				...
				if (defaultExpressionEngine$1.isExpression(def)) return {};
		...
		const operationDefinitions = {
			where(results) {
				...
				return results.filter((result) => {
					return defaultExpressionEngine$1.apply(whereExpression, result);
				});
			},
			order(results) {
