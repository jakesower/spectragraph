...
import { DataTree, Query } from "../types";
...
export class ScopedTree implements ScopedTreeType {
  public readonly attributes: { [k: string]: unknown };
  ...
  constructor(schema: SchemaType, query: Query, dataTree: DataTree) {
    ...
    const scope = (subTreeAttrs, subQuery: Query) => {
      ...
      const scopedRels = mapObj(
        ...
        (relQuery, relName) => {
          ...
          return applyOrMap(subTreeAttrs[relName], (relAttrs) => scope(relAttrs, nextQuery));
        },
      );
