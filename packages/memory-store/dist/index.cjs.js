...
function setInverseRelationships(oldResource, newResource, context) {
	...
	esToolkit.mapValues(resSchema.relationships, (_, relName) => {
...
function create(resource, context) {
	...
	const base = createResource(schema, { type, id: id ?? uuid.v4() });
...
function deleteAction(resource, context) {
	...
	esToolkit.mapValues(resSchema.relationships, (relSchema, relName) => {
		setInversesOnRelationship(
			existingRes,
...
function merge(resourceOrResources, context) {
	...
	const nextGraph = esToolkit.mapValues(storeGraph, (g) => ({ ...g }));
	const expectedToBeCreated = esToolkit.mapValues(storeGraph, () => new Set());
	const created = esToolkit.mapValues(storeGraph, () => new Set());

	// NOTE: if an error is thrown, any change that have been made are discarded (rollback)
	const outputs = utils.applyOrMap(resourceOrResources, (rootResource) => {
		ensureValidMergeResource(schema, rootResource, { validator });

		const go = (resource) => {
			const { type, id = uuid.v4() } = resource;
			...
			const nextRels = esToolkit.mapValues(
				resource.relationships ?? {},
				(relOrRels, relName) => {
					const preppedRels = utils.applyOrMap(relOrRels, (rel) => {
			...
			nextGraph[type][id] = {
				...nextGraph[type][id],
				relationships: esToolkit.mapValues(nextGraph[type][id].relationships, (r) =>
