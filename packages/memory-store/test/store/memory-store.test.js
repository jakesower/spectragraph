...
describe("update validation", () => {
	...
	it("fails validation on an invalid type", () => {
		...
		expect(() => store.update({ type: "foo" })).toThrowError();
	});
...
describe("delete validation", () => {
	it("fails validation on an empty object", () => {
		...
		expect(() => store.delete({})).toThrowError();
	});
	...
	it("fails validation on an invalid type", () => {
		...
		expect(() => store.delete({ type: "foo" })).toThrowError();
	});
...
describe("splice", () => {
	describe("without an id", () => {
		it("fails validation on an empty object", () => {
			...
			expect(() => store.splice({})).toThrowError();
		});
		...
		it("works with a valid new relationship", () => {
			...
			expect(result.id).not.toEqual(null);
			expect(result.relationships.homeTeam.id).not.toEqual(null);
			expect(result).toMatchObject({
				type: "games",
				attributes: { homeScore: 5, awayScore: 1 },
				relationships: {
...
describe("custom schemas", () => {
	...
	const createCustomMemoryStore = () =>
		createMemoryStore(customSchema, {
