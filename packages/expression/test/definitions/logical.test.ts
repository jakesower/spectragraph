...
const { compile } = expressionContext(comparativeDefinitions);
...
it("implements the $in expression", () => {
	const exp = { $pipe: [{ $prop: "age" }, { $in: [4, 6] }] };
...
it("implements the $nin expression", () => {
	const exp = { $pipe: [{ $prop: "age" }, { $nin: [4, 6] }] };
