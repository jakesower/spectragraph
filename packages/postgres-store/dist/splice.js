import { validateResourceTree } from "data-prism";
...
export async function splice(resource, context) {
    ...
    const go = async (res) => {
        ...
        await Promise.all(Object.entries(res.relationships ?? {}).map(async ([relName, relVal]) => {
            if (Array.isArray(relVal)) {
                const processed = await Promise.all(relVal.map(go));
                processedRelationships[relName] = await Promise.all(processed.map((p) => upsertForeignRelationshipRows(p, context)));
            }
            else {
                const processed = await go(relVal);
                processedRelationships[relName] = upsertForeignRelationshipRows(processed, context);
            }
        }));
        return {
            type: res.type,
