 ...
 */
const whereExpressionEngine = expressions.createExpressionEngine(
	...
	where: (where, { table }) => {
		const propExprs = Object.entries(where).map(([propKey, propValOrExpr]) => {
			...
			if (whereExpressionEngine.isExpression(propValOrExpr)) {
				...
				return { [operation]: [`${table}.${lodashEs.snakeCase(propKey)}`, args] };
			}

			return { $eq: [`${table}.${lodashEs.snakeCase(propKey)}`, propValOrExpr] };
		});
	...
	const extractors = flatMapQuery(schema, rootQuery, (_, info) => {
		...
		return (result) => {
			...
			if (parentQuery) {
				...
				const parentIdPath = `${rootQuery.type}${parentPathStr}.${lodashEs.snakeCase(
					parentIdAttribute,
				)}`;
			...
			if (attributes.length > 0) {
				attributes.forEach((attr) => {
					const fullAttrPath = `${rootQuery.type}${pathStr}.${lodashEs.snakeCase(attr)}`;
		...
		toSql: (val) => {
			...
			const orderClauses = val.map(
				({ property, direction, table }) =>
					`${table}.${lodashEs.snakeCase(property)}${
						direction === "desc" ? " DESC" : ""
					}`,
			);
...
async function create(resource, context) {
	...
	const idColumns = resource.id ? [lodashEs.snakeCase(idAttribute)] : [];
...
async function deleteResource(resource, context) {
	...
	const sql = `
    DELETE FROM ${table}
    WHERE ${lodashEs.snakeCase(idAttribute)} = $1
		RETURNING *
  `;
...
async function update(resource, context) {
	...
	if (columnsWithPlaceholders.length > 0) {
		const sql = `
			...
			WHERE ${lodashEs.snakeCase(idAttribute)} = $${columns.length + 1}
			RETURNING *
		`;
		...
		const { rows } = await db.query(
			`SELECT * FROM ${table} WHERE ${lodashEs.snakeCase(idAttribute)} = $1`,
			[resource.id],
		);
		firstResult = rows[0];
	}
...
async function getOne(type, id, context) {
	...
	const cols = [
		...
		...localRelationships.map(([_, r]) => lodashEs.snakeCase(r.localColumn)),
	].join(", ");
