...
var didYouMean$1 = /*@__PURE__*/getDefaultExportFromCjs(didYouMean1_2_1Exports$1);
...
const $eq$1 = createComparativeExpression$1((a, b) => esToolkit.isEqual(a, b));
const $ne$1 = createComparativeExpression$1((a, b) => !esToolkit.isEqual(a, b));
...
var didYouMean = /*@__PURE__*/getDefaultExportFromCjs$1(didYouMean1_2_1Exports);
...
const whereExpressionEngine = createExpressionEngine(
	esToolkit.mapValues(sqlExpressions, (expr) => ({ ...expr, evaluate: expr.where })),
);
...
const varsExpressionEngine = createExpressionEngine(
	esToolkit.mapValues(sqlExpressions, (expr) => ({ ...expr, evaluate: expr.vars })),
);
...
async function create(resource, context) {
	...
	const idColumns = resource.id ? [esToolkit.snakeCase(idAttribute)] : [];
...
async function deleteResource(resource, context) {
	...
	const sql = `
    DELETE FROM ${table}
    WHERE ${esToolkit.snakeCase(idAttribute)} = $1
		RETURNING *
  `;
...
async function update(resource, context) {
	...
	if (columnsWithPlaceholders.length > 0) {
		const sql = `
			...
			WHERE ${esToolkit.snakeCase(idAttribute)} = $${columns.length + 1}
			RETURNING *
		`;
		...
		const { rows } = await client.query(
			`SELECT * FROM ${table} WHERE ${esToolkit.snakeCase(idAttribute)} = $1`,
			[resource.id],
		);
		firstResult = rows[0];
	}
...
async function merge(resourceTreeOrTrees, context) {
	...
	const go = async (resource) => {
		const processedRels = await utils.promiseObjectAll(
			esToolkit.mapValues(resource.relationships ?? {}, (relOrRels) =>
				utils.applyOrMap(relOrRels, (rel) => {
	...
	const result = await Promise.all(utils.applyOrMap(resourceTrees, go));
...
function createPostgresStore(schema, config) {
	...
	return {
		...
		async merge(resourceTreeOrTrees) {
			utils.applyOrMap(resourceTreeOrTrees, (tree) =>
